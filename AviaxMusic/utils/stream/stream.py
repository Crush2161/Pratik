...
async def search_telegram_files(title):
    # Implement logic to search for files in Telegram
    return None  # Return the file if found, otherwise return None

async def stream_from_telegram_file(file, chat_id, original_chat_id, user_name):
    await app.send_message(original_chat_id, f"Streaming from Telegram file: {file}")
    ...
    try:
        if streamtype == "playlist":
            ...
            for search in result:
                ...
                if await is_active_chat(chat_id):
                    ...
                    try:
                        ...
                        telegram_file = await search_telegram_files(title)
                        if telegram_file:
                            await stream_from_telegram_file(telegram_file, chat_id, original_chat_id, user_name)
                            return
                        else:
            ...
            if await is_active_chat(chat_id):
                ...
                await run.delete()  # Delete the message after 60 seconds
